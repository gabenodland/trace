 ...
 * SettingsDrawer
 *
 * Animated drawer component for View/Sort/Filter settings.
 * Renders at app root level as an overlay on the RIGHT side.
 *
 * Features:
 * - Animated slide-in from right
 * - Backdrop with fade
 * - Swipe-to-close gesture (swipe left on drawer)
...
import { useSettingsDrawer, type SettingsDrawerControl } from "../../shared/contexts/SettingsDrawerContext";
...
export function SettingsDrawer() {
  ...
  const { isOpen, openDrawer, closeDrawer, registerDrawerControl } = useSettingsDrawer();

  // Animation values - drawer slides from right (starts at +DRAWER_WIDTH, opens to 0)
  ...
  useEffect(() => {
    const control: SettingsDrawerControl = {
      setPosition,
      animateOpen,
      animateClose,
      getDrawerWidth,
  ...
  // Swipe-to-close gesture on drawer itself - swipe LEFT to close (opposite of StreamDrawer)
  const panResponder = useRef(
    PanResponder.create({
      // Don't capture initial touch - let children handle taps
      onStartShouldSetPanResponder: () => false,
      onMoveShouldSetPanResponder: (_, gestureState) => {
        // Only capture horizontal swipes to the right (closing direction for right drawer)
  ...
  return (
    <View style={styles.container} pointerEvents={isActive ? "auto" : "none"}>
      ...
      {/* Drawer panel - positioned on RIGHT side */}
      <Animated.View
        style={[
          styles.drawer,
...
const styles = StyleSheet.create({
  ...
  drawer: {
    ...
    // Shadow on LEFT edge (opposite of StreamDrawer)
    shadowColor: "#000",
